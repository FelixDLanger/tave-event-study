...
def collect_edgar(firm, fetch_body=True) -> list:
    ...
    Two important changes from the naive version:
      ...
      2. For each candidate, fetch the actual filing text and classify on THAT,
         not on metadata. EDGAR full-text search returns no usable headline, so
         classifying on metadata produced 100% 'Unclassified'.

    To keep runtime sane, body fetch is capped: we only fetch for filings whose
    metadata already matched a tokenization keyword (which is the search itself).
    """
    rows = []
    ...
    # Tighter keyword set for EDGAR: specific tokenization terms only.
    # Generic 'blockchain'/'DLT'/'digital asset' alone are too noisy in bank filings.
    edgar_keywords = [
        '"asset tokenization"', '"tokenized fund"', '"tokenised fund"',
        '"tokenized bond"', '"tokenized treasury"', '"tokenized deposit"',
        '"digital bond"', '"tokenized securities"', '"tokenised securities"',
        '"real-world asset"', '"real world asset tokeniz"', '"on-chain fund"',
    ...
    for kw in edgar_keywords:
        try:
            params = {
                ...
                "entity": firm["name"],
                "forms": "8-K,6-K",   # MATERIAL events only — not 10-K/10-Q boilerplate
            }
            ...
            for hit in hits:
                ...
                # Fetch the actual filing text to classify on real content
                body = ""
                if fetch_body:
                    body = _fetch_edgar_doc_text(cik_num, acc)

                # Headline: use the keyword phrase + form type as a proxy title,
                # plus the body for classification
                kw_clean = kw.strip('"')
                pseudo_headline = f"{firm['name']} {src.get('form_type','8-K')} {kw_clean}"
                rows.append(_base_row(
                    firm, kw_clean, date, src.get("form_type", "8-K"), acc,
