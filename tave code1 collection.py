...
if __name__ == "__main__":
    ...
    # CLEAN SGX/SIX SCRAPE (recommended next run):
    #   Keeps US served from cache (fast — no 16-min EDGAR re-scrape), and forces
    #   SGX + SIX to scrape fresh so the Singapore/Switzerland events repopulate
    #   deterministically regardless of what's in the Drive Excel right now.
    #   The dtype fix makes the cached-US + fresh-SGX/SIX merge complete cleanly,
    #   so all three markets land in tave_master_database.xlsx and flow to Code 2.
    run_collection(refresh=("SGX", "SIX"))

    # Full re-scrape of everything (slow — only if you want fresh US too):
    # run_collection(refresh=("US", "SGX", "SIX"))
